...
void cdc_gen_string_serial(UINT8 len)
{
	...
	E_DIS = 1;
	ROM_ADDR = ROM_CHIP_ID_LO;
	ROM_CTRL = ROM_CMD_RD_OTP;
	SAFE_MOD = 0x00;	//NOP... Not sure if needed
	E_DIS = 0;
	...
	while(len)
	{
		...
		{
			if(cdc_string_serial_offset & 0x08)
				chip_id = ROM_DATA_HI;
			else
				chip_id = ROM_DATA_LO;
			if(cdc_string_serial_offset & 0x04)
				chip_id = chip_id >> 8;
			id_hex = (UINT8)chip_id;
			
			if(cdc_string_serial_offset & 0x02)
				id_hex = id_hex >> 4;
			id_hex &= 0x0F;
			if(id_hex < 10)
				id_hex += '0';
