...
#ifndef _STM32F205_CORE_H_
#define _STM32F205_CORE_H_
...
#define SysTick_CLKSource_HCLK_Div8	0x00000000UL
#define SysTick_CLKSource_HCLK		0x00000004UL

#define core_systick_clk_src_config(clk_src) (SysTick->CTRL = (SysTick->CTRL & ~SysTick_CTRL_CLKSOURCE_Msk) | (clk_src))

__attribute__( ( always_inline ) ) static inline void core_enable_global_irq()               { __asm volatile ("cpsie i"); }
__attribute__( ( always_inline ) ) static inline void core_disable_global_irq()              { __asm volatile ("cpsid i"); }

__attribute__( ( always_inline ) ) static inline void core_enable_fault_irq()         { __asm volatile ("cpsie f"); }
__attribute__( ( always_inline ) ) static inline void core_disable_fault_irq()        { __asm volatile ("cpsid f"); }

__attribute__( ( always_inline ) ) static inline void core_nop()                      { __asm volatile ("nop"); }
__attribute__( ( always_inline ) ) static inline void core_wfi()                      { __asm volatile ("wfi"); }
__attribute__( ( always_inline ) ) static inline void core_wfe()                      { __asm volatile ("wfe"); }
__attribute__( ( always_inline ) ) static inline void core_sev()                      { __asm volatile ("sev"); }
__attribute__( ( always_inline ) ) static inline void core_isb()                      { __asm volatile ("isb"); }
__attribute__( ( always_inline ) ) static inline void core_dsb()                      { __asm volatile ("dsb"); }
__attribute__( ( always_inline ) ) static inline void core_dmb()                      { __asm volatile ("dmb"); }
__attribute__( ( always_inline ) ) static inline void core_clrex()                    { __asm volatile ("clrex"); }
...
static inline void core_nvic_set_priority_grouping(uint32_t PriorityGroup)
{
  uint32_t reg_value;
...
static inline uint32_t core_nvic_get_priority_grouping(void)
{
...
static inline void core_nvic_Enable_irq(IRQn_Type IRQn)
{
 ...
 * The interrupt number cannot be a negative value. */
static inline void core_nvic_disable_irq(IRQn_Type IRQn)
{
  NVIC->ICER[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* disable interrupt */
}

/* Read the pending register in NVIC and return 1 if its status is pending, otherwise it returns 0 */
static inline uint32_t core_nvic_get_pending_irq(IRQn_Type IRQn)
{
 ...
 * The interrupt number cannot be a negative value. */
static inline void core_nvic_set_pending_irq(IRQn_Type IRQn)
{
 ...
 * The interrupt number cannot be a negative value. */
static inline void core_nvic_clear_pending_irq(IRQn_Type IRQn)
{
  NVIC->ICPR[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* Clear pending interrupt */
}

/* Read the active register in NVIC and returns 1 if its status is active, otherwise it returns 0. */
static inline uint32_t core_nvic_get_active(IRQn_Type IRQn)
{
...
static inline void core_nvic_set_priority(IRQn_Type IRQn, uint32_t priority)
{
  if(IRQn < 0) {
 ...
 *
 * Note: The priority cannot be set for every core interrupt. */
static inline uint32_t core_nvic_get_priority(IRQn_Type IRQn)
{
...
static inline uint32_t core_nvic_encode_priority (uint32_t PriorityGroup, uint32_t PreemptPriority, uint32_t SubPriority)
{
  uint32_t PriorityGroupTmp = (PriorityGroup & 0x07);          /* only values 0..7 are used          */
...
static inline void core_nvic_decode_priority (uint32_t Priority, uint32_t PriorityGroup, uint32_t* pPreemptPriority, uint32_t* pSubPriority)
{
  uint32_t PriorityGroupTmp = (PriorityGroup & 0x07);          /* only values 0..7 are used */
  uint32_t PreemptPriorityBits;
  uint32_t SubPriorityBits;
...
static inline void core_nvic_system_reset(void)
{
  SCB->AIRCR  = ((0x5FA << SCB_AIRCR_VECTKEY_Pos)      |
                 (SCB->AIRCR & SCB_AIRCR_PRIGROUP_Msk) |
                 SCB_AIRCR_SYSRESETREQ_Msk);                   /* Keep priority group unchanged */
  core_dsb();                                                     /* Ensure completion of memory access */
  while(1);                                                    /* wait until reset */
}

/* Initialize the system tick timer and its interrupt and start the
...
static inline uint32_t SysTick_Config(uint32_t ticks)
{
  ...
  core_nvic_set_priority (SysTick_IRQn, (1<<__NVIC_PRIO_BITS) - 1);  /* set Priority for Cortex-M0 System Interrupts */
