...
volatile UINT16 timer_overflow_counts[4];
...
void timer3_isr(void) interrupt INT_NO_TMR3
{
	++timer_overflow_counts[3];
	if(timer3_callback)
		timer3_callback();
	T3_STAT |= bT3_IF_END;
}
...
void timer_init(UINT8 timer, void (*callback)(void))
{
	switch(timer)
	{
		...
		case TIMER_3:
			...
			T3_SETUP &= ~bT3_EN_CK_SE;
			T3_CTRL = 0x00;
			timer_overflow_counts[3] = 0;
			timer3_callback = callback;
			T3_SETUP |= bT3_IE_END;
			IE_TMR3 = 1;	//enable interrupts
			break;
		default:
			return;
	}
