...
void uart0_isr(void) interrupt INT_NO_UART0
{
    ...
    {
        RI = 0;
        fifo_push(uart0_rx_fifo_ptr, SBUF);
    }
    if(TI)
    {
        TI = 0;
...
{
	UINT8 int_status = SER1_IIR & MASK_U1_IIR_INT;
	UINT8 num_to_write;
	if(int_status == U1_INT_RECV_RDY)
    {
        fifo_push(uart_rx_fifos[UART_1], SER1_RBR);
		return;
    }
    if(int_status == U1_INT_THR_EMPTY)
    {
		num_to_write = fifo_num_used(uart_tx_fifos[UART_1]);
		if(!num_to_write)
		{
		...
		while(num_to_write)
		{
			SER1_THR = fifo_pop(uart_tx_fifos[UART_1]);
			--num_to_write;
		}
    }
}
...
void uart1_init(UINT32 baud, UINT8 pin_map)
{
	...
	SER1_LCR = 0x83;
	SER1_DIV = 0x01;
	baud_div = FREQ_SYS / 8ul / baud;
	SER1_DLL = (UINT8)baud_div;
	SER1_DLM = (UINT8)(baud_div >> 8);
	SER1_LCR = 0x03;
	SER1_FCR = 0x07;
	SER1_MCR = bMCR_OUT2;
	SER1_IER = 0x03 | pin_map;
...
void uart_write_byte(UINT8 uart, UINT8 val)
{
	...
	{
		IE_UART1 = 0;
		fifo_push(uart_tx_fifos[UART_1], val);
		SER1_IER |= bIER_THR_EMPTY;
		IE_UART1 = 1;
	}
	else
	{
		...
		{
			fifo_push(uart_tx_fifos[UART_0], val);
		}
		ES = 1;
	}
}
...
void uart_write_bytes(UINT8 uart, UINT8* src, UINT16 num_bytes)
{
	...
	if(uart)
	{
		while(num_bytes)
		{
			num_to_write = fifo_num_free(uart_tx_fifos[UART_1]);
			if(!num_to_write)
				continue;
			...
			IE_UART1 = 0;
			fifo_write(uart_tx_fifos[UART_1], src, num_to_write);
			SER1_IER |= bIER_THR_EMPTY;
			IE_UART1 = 1;
	...
	else
	{
		...
		while(num_bytes)
		{
			num_to_write = fifo_num_free(uart_tx_fifos[UART_0]);
			if(!num_to_write)
				continue;
			if(num_to_write > num_bytes)
			{
			...
			ES = 0;
			fifo_write(uart_tx_fifos[UART_0], src, num_to_write);
			ES = 1;
			num_bytes -= num_to_write;
			src += num_to_write;
		}
