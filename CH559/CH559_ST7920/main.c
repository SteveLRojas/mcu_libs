...
int main()
{
	...
	gpio_set_pin_mode(GPIO_MODE_OUTPUT_PP, GPIO_PORT_2, GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3);
		...
		if(cdc_bytes_available() >= 2)
		{
			...
			switch(RESET_KEEP)
			{
				...
				case 0x1D:
					...
					{
						st7920_set_pixel(temp, temp, 0x01);
					}
					st7920_update_display();
					break;
				case 0x1E:
					...
					{
						st7920_set_pixel(temp >> 2, temp, 0x01);
					}
					st7920_update_display();
					break;
				case 0x1F:
					for(temp = 0; temp < 128; ++temp)
					{
						st7920_set_pixel(temp >> 1, temp, 0x01);
					}
					st7920_update_display();
					break;
				default:
